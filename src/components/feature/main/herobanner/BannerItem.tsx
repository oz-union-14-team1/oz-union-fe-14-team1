    ...
    <>
      ...
      <div
        className={`absolute text-white ${
          isDesktop ? 'bottom-8 left-8' : 'right-4 bottom-10 left-4'
        }`}
      >
        <div className="flex gap-1">
          {banner.tag.map((tag) => (
            <p
              key={tag}
              className={`${isDesktop ? 'text-md mb-2' : 'mb-1 text-sm'}`}
            >
              {tag}
            </p>
          ))}
        ...
        <h2
          className={`font-bold ${isDesktop ? 'mb-3 text-4xl' : 'mb-2 text-2xl'}`}
        >
          {banner.title}
        </h2>
